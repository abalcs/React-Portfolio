...
const CACHE_FACTS = [
  'Every tree, rock, and ripple here is generated by code — the whole mountain fits in a few kilobytes of math.',
  'The mountain is seeded — every visitor climbs this exact same peak.',
  'The entire forest renders in two draw calls. All ~380 trees of it.',
  'The summit flag has all 50 stars, drawn one by one onto a canvas.',
  'No WebGL? This site quietly serves a full 2D version instead.',
];

/** Ranger mailbox by the trailhead — leave a note (opens Contact). */
function Mailbox({ onSelect }: { onSelect: (s: SectionId) => void }) {
  const position = useMemo(() => besideTrail(0.035, -3.4), []);
  const [hovered, setHovered] = useState(false);
  const inner = useRef<THREE.Group>(null);
  useFrame((_, delta) => {
    if (!inner.current) return;
    const s = THREE.MathUtils.damp(inner.current.scale.x, hovered ? 1.15 : 1, 12, delta);
    inner.current.scale.setScalar(s);
  });
  return (
    <group position={position}>
      ...
      <group ref={inner}>
        ...
        <mesh position={[0, 1.18, 0]}>
          ...
          <meshStandardMaterial color="#2e4a3d" metalness={0.4} roughness={0.5} />
        </mesh>
        {/* red flag up — you've got mail to send */}
        <mesh position={[0.2, 1.32, -0.12]}>
...
/** Far-hill radio tower with a blinking beacon — broadcasts GitHub stats. */
function RadioTower({ onSelect }: { onSelect: (s: SectionId) => void }) {
  const position = useMemo(() => {
    const x = -175;
    const z = -15;
    return new THREE.Vector3(x, gridHeight(x, z), z);
  }, []);
  ...
  useFrame(({ clock }) => {
    if (beacon.current) {
      beacon.current.emissiveIntensity =
        Math.sin(clock.elapsedTime * 2.4) > 0.4 ? 2.6 : 0.15;
    }
  });
  return (
    <group position={position}>
      ...
      >
        <boxGeometry args={[4, 19, 4]} />
        <meshBasicMaterial transparent opacity={0} depthWrite={false} />
      </mesh>
      {/* tapered lattice segments */}
      {[0, 1, 2].map((i) => (
        <mesh key={i} position={[0, 3 + i * 5.6, 0]}>
          <cylinderGeometry args={[0.55 - i * 0.16, 0.85 - i * 0.16, 5.8, 4, 1, true]} />
          <meshStandardMaterial
            color={hovered ? '#e0433a' : '#b8352d'}
            metalness={0.6}
            roughness={0.45}
            wireframe
          />
      ...
      <mesh position={[0, 17.4, 0]}>
        ...
        <meshStandardMaterial color="#8f959c" metalness={0.8} roughness={0.3} />
      </mesh>
...
/** Hidden geocache — an old ammo box with rotating site trivia. */
function Geocache() {
  const position = useMemo(() => besideTrail(0.55, 27), []);
  ...
  return (
    <group position={position}>
      <mesh
        ...
        onClick={(e) => {
          e.stopPropagation();
          toast(CACHE_FACTS[factIndex.current % CACHE_FACTS.length], {
            icon: '📦',
            duration: 6000,
          });
          factIndex.current++;
      ...
      <mesh position={[0, 0.18, 0]} scale={hovered ? 1.1 : 1}>
        <boxGeometry args={[0.6, 0.34, 0.38]} />
        <meshStandardMaterial color="#4a5d3a" metalness={0.5} roughness={0.55} />
      </mesh>
      <mesh position={[0, 0.38, 0]} scale={hovered ? 1.1 : 1}>
        <boxGeometry args={[0.63, 0.08, 0.41]} />
        <meshStandardMaterial color="#3c4c30" metalness={0.5} roughness={0.55} />
      </mesh>
    </group>
  );
}

/** Fishing spot on the lake shore — pure scenery. */
function FishingSpot() {
  ...
  return (
    <group position={position} rotation={[0, Math.PI / 4, 0]}>
      {/* rod propped on a forked stick, line toward the water */}
      <mesh position={[0, 0.35, 0]} rotation={[0, 0, 0.3]}>
        <cylinderGeometry args={[0.025, 0.035, 0.7, 5]} />
        <meshStandardMaterial color="#5d4429" roughness={0.9} />
      </mesh>
      <mesh position={[0.4, 0.75, 0]} rotation={[0, 0, -0.9]}>
        <cylinderGeometry args={[0.015, 0.025, 2.4, 6]} />
        <meshStandardMaterial color="#3a2f22" roughness={0.7} />
      </mesh>
      {/* bait bucket */}
      <mesh position={[-0.5, 0.22, 0.2]}>
        <cylinderGeometry args={[0.2, 0.17, 0.42, 10]} />
        <meshStandardMaterial color="#7a2e2e" metalness={0.4} roughness={0.5} />
      </mesh>
    </group>
  );
}
